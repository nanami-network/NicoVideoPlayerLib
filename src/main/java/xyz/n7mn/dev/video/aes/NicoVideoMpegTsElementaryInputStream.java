...
public class NicoVideoMpegTsElementaryInputStream extends InputStream {
    private static final Logger log = LoggerFactory.getLogger(NicoVideoMpegTsElementaryInputStream.class);
     ...
     */
    public NicoVideoMpegTsElementaryInputStream(InputStream inputStream, int elementaryDataType) {
    ...
    public int read() throws IOException {
        if (!findElementaryData()) {
            final int skipBytes = packet[0];
            if (skipBytes == -1 || this.inputStream.skip(skipBytes) != skipBytes) {
                return -1;
            }
            return read();
        }
